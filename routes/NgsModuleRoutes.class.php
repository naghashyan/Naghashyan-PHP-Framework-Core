...
namespace ngs\framework\routes {
    class NgsModuleRoutes {
        ...
        public function getShuffledRoutes() {
            ...
            $modulePart = $this->getModulePartByDomain(NGS()->getHttpUtils()->getMainDomain());
            foreach ($modulePart as $routTypeName => $routesType) {
                foreach ((array)$routesType as $key => $value) {
                    if (isset($value["dir"])) {
                        $this->shuffledRoutes[$routTypeName][$value["dir"]] = $key;
                    }
                }
            }
            return $this->shuffledRoutes;
